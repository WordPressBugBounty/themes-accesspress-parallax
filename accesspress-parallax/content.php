...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'article-wrap' ); ?>>
    ...
    <header class="entry-header">
        <?php
        $show_date = accesspress_parallax_of_get_option( 'bl_post_date', '1' ); 
        if($show_date){
    ...
    <div class="entry-content">
        <?php $excerpt_length = accesspress_parallax_of_get_option( 'blog_excerpt_length','150' );
            echo accesspress_parallax_excerpt_content( $excerpt_length ); ?>
        <?php
        wp_link_pages( array(
