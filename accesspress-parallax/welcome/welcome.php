			...
			public function welcome_admin_notice_display() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					<div class="updated apwelcome-an notice notice-success is-dismissible">
						<h2><?php printf(esc_html__( 'Welcome!', 'accesspress-parallax' )) ?></h2>
						<p class="about-desc"><?php /* translators: %s : Theme Name */ printf(esc_html__( 'Thank you for installing %1$s. It is now installed and ready to use. Here are some useful links to get started: ', 'accesspress-parallax' ), $this->theme_name);  ?></p>
						<hr/>
							...
							<div class="apwelcome-column col-doc">
								<div>
									<h3>
										...
										<?php printf(esc_html__( 'Documentation', 'accesspress-parallax' )) ?>
									</h3>
			...
			public function welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
								...
								<div class="welcome-section-wrapper is_loading">
									...
									<div class="notice-sidebar is_loading">
										<div class= "notice-sidebar-item">
											<h4><?php echo esc_html('Join in our social networks!','accesspress-parallax') ?></h4>
											<p><?php echo esc_html__('Get connected, share your opinions and more via our social community: ', 'accesspress-parallax'); ?></p>

											<p><a href="<?php echo esc_url('https://www.facebook.com/AccessPressThemes')?>"><?php echo esc_html('Join our Facebook Group','accesspress-parallax') ?></a><?php echo esc_html__(' -to receive updates, offers and more.', 'accesspress-parallax'); ?></p>

											<p><a href="<?php echo esc_url('https://www.youtube.com/channel/UCzD8I1moKISXDjAVr8dIf-w')?>"><?php echo esc_html('Subscribe our YouTube Channel','accesspress-parallax') ?></a><?php echo esc_html__(' -for tutorials, videos and more.', 'accesspress-parallax'); ?></p>

											<p><a href="<?php echo esc_url('https://twitter.com/apthemes')?>"><?php echo esc_html('Follow us on Twitter','accesspress-parallax') ?></a><?php echo esc_html__(' -to stay updated. ', 'accesspress-parallax'); ?></p>
										</div>
										<div class= "notice-sidebar-item">
											<h4><?php echo esc_html('Rate us!','accesspress-parallax') ?></h4>
											<p><?php /* translators: %s : Theme Name */ printf(esc_html__('Are you enjoying %1$s? We would love to hear your feedback!', 'accesspress-parallax'),$this->theme_name); ?></p>
											<a href="https://wordpress.org/support/theme/accesspress-parallax/reviews/#new-post"><?php echo esc_html('Write a review','accesspress-parallax') ?></a>
					...
					public function welcome_styles_and_scripts() {
						...
						wp_localize_script( $this->theme_slug . '-welcome-screen', 'SmWelcomeObject', array(
							...
							'offline_install_nonce'	=> wp_create_nonce( 'offline_install_nonce'),
				...
				// Get Plugin Info
					...
					public function plugin_offline_installer_callback() {
						...
						$nonce = isset( $_POST["nonce"] ) ? sanitize_text_field( wp_unslash( $_POST["nonce"] ) ) : '';
						// Check our nonce, if they don't match then bounce!
						if (! wp_verify_nonce( $nonce, 'offline_install_nonce' )) {
							wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'accesspress-parallax') );
						}
					...
					public function plugin_activation_callback() {
						...
						if(file_exists($plugin_file)) {
							...
							echo esc_html__('Plugin Does not Exists' , 'accesspress-parallax');
						}
					...
					public function plugin_deactivation_callback() {
						if ( ! current_user_can('activate_plugins') ) {
							wp_die( esc_html__( 'Sorry, you are not allowed to deactivate plugins on this site.', 'accesspress-parallax' ) );
						}
						$nonce = isset( $_POST["nonce"] ) ? sanitize_text_field( wp_unslash( $_POST["nonce"] ) ) : '';
						// Check our nonce, if they don't match then bounce!
